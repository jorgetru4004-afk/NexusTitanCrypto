...
async function fetchFundingRates() {
  try {
    // Binance public funding rate endpoint (no auth needed)
    const resp = await axios.get('https://fapi.binance.com/fapi/v1/premiumIndex', { timeout: 8000 });
    const rates = {};
    for (const item of (resp.data || [])) {
      if (item.symbol && item.lastFundingRate) {
        const rate = parseFloat(item.lastFundingRate) * 100; // convert to percentage
        const base = item.symbol.replace('USDT', '').replace('BUSD', '');
        rates[base] = { rate: parseFloat(rate.toFixed(4)), symbol: item.symbol, time: new Date().toISOString() };
      }
    }
    // Store key coins
    const keyCoins = ['BTC', 'ETH', 'SOL', 'AVAX', 'LINK', 'AAVE', 'UNI', 'DOGE', 'MATIC', 'DOT'];
    for (const coin of keyCoins) {
      if (rates[coin]) fundingRates[coin] = rates[coin];
    }
    ...
    if (extremes.length > 0) console.log(`💰 Extreme funding rates: ${extremes.join(' ')}`);
  } catch (e) {
    console.log('Funding rates unavailable:', e.message);
  }
}
